...
class Chromosome:
    ...
    def toStudentsTimetableHtml(self):
        flat = self.timetable.reshape((self.num_Batch, self.classesInDay, self.days))
        tables = {}
            ...
            for i in range(flat.shape[1]):
                for j in range(flat.shape[2]):
                    ...
                    if geneId != -1:
                        ...
                        if(tables[s][i, j] != -1):
                            print("Error ", i, j, gene.subject, self.searchGene(abs(tables[s][i, j])).subject)
            ...
            for i in range(table.shape[0]):
                ...
                for j in range(table.shape[1]):
                    ...
                    if geneId == -1:
                        ...
                        td = "<td class='bg-danger'><span class='course'>{0}</span><hr><span class='teacher'>{1}</span></td>".format(self.genes[-geneId].subject, self.genes[-geneId].prof_name)
                    else:
                        td = "<td><span class='course'>{0}</span><hr><span class='teacher'>{1}</span></td>".format(self.genes[geneId].subject, self.genes[geneId].prof_name)
                    tr += td 
                tr += '</tr>'
                stable += tr
            stable += "</table>"
